...
parentPath = os.path.join(root, 'dataset_generator', 'frames')
print(parentPath)
...
def test_image_plotting():
    for movie in os.listdir(parentPath):
        count = 0
        for image in os.listdir(os.path.join(parentPath, movie)):
            image_path = os.path.join(parentPath, movie, image)
            print(image_path)
